...
def run_original_simulation():
    ...
    # Force default clean light styling globally
    plt.rcParams.update(plt.rcParamsDefault)
    plt.style.use('default')

    # ---- Setup Mass Range Data Sweep ----
    ...
    plt.tight_layout()
    plt.savefig('results/Fig1_Emax.png', dpi=300)
    plt.close()

    # ---- Figure 2: QCF Lifetimes ----
    ...
    plt.legend()
    plt.title('QCF lifetimes')
    plt.grid(True, which='both', alpha=0.3)
    plt.tight_layout()
    plt.savefig('results/Fig2_lifetime.png', dpi=300)
    plt.close()

    # ---- Figure 3: Spectrum M = 5e11 kg ----
    ...
    plt.xlabel('E [eV]')
    plt.ylabel('flux')
    plt.title('Spectrum M=5e11 kg')
    plt.legend()
    plt.grid(True, which='both', alpha=0.3)
    plt.tight_layout()
    plt.savefig('results/Fig3_spectrum.png', dpi=300)
    plt.close()

    # ---- Figure 4: K/Kmax Solar Mass Curve ----
    print("[Generating]: Figure 4 -> results/Fig4_KKmax.png")
    M_sun = 1.98847e30
    rs = r_s(M_sun)
    rc = r_cut(M_sun)
    r = np.logspace(np.log10(rc * 0.9), np.log10(20 * rs), 500)
    ratio = K_schw(r, M_sun) / K_max
    ratio_c = np.minimum(ratio, 1)

    plt.figure(figsize=(8, 5))
    plt.loglog(r / rs, ratio, '--', label='GR', color='C0')
    plt.loglog(r / rs, ratio_c, lw=2, label='QCF', color='C1')
    plt.axvline(rc / rs, c='r', ls=':')
    plt.axhline(1, c='k', lw=1)
    plt.xlabel('r/r_s')
    plt.ylabel('K/K_max')
    plt.title('Solar-mass curvature ceiling')
    plt.legend()
    plt.grid(True, which='both', alpha=0.3)
    plt.tight_layout()
    plt.savefig('results/Fig4_KKmax.png', dpi=300)
    plt.close()

    # ---- Figure 5: Interior Truncation Plot (Light Theme Matched) ----
    print("[Generating]: Figure 5 -> results/QCF_Kretschmann_Truncation.png")
    ...
    k_smooth = (12 * r_s_norm**2) / (r_vals**6 + (12 * r_s_norm**2 / K_max_norm))
    
    # Styled using the standardized light background color schema
    ...
    plt.plot(r_vals, k_smooth, label='QCF Smooth Balanced Core', color='C1', linewidth=2)
    ...
    plt.xlabel('Radial Coordinate $r$', fontsize=13)
    plt.ylabel('Kretschmann Scalar $K$', fontsize=13)
    plt.xlim(-0.1, 4.2)
    plt.ylim(0, 0.125)
    plt.grid(True, linestyle='-', alpha=0.2)
    plt.legend(loc='upper right', framealpha=0.9)
    plt.tight_layout()
    plt.savefig('results/QCF_Kretschmann_Truncation.png', dpi=300)
    plt.close()

    print("\n==========================================================")
    print(" SUCCESS: All figures perfectly synced to results folder.")
    print("==========================================================")
